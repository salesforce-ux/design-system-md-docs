...
| .slds-is-grabbed | grabbed state of a listbox option |   | .slds-dueling-list__options div |   |   |
| .slds-is-selected | Selected/dragging state of a listbox option |   | .slds-dueling-list__options div |   |   |
...
| .slds-dueling-list__options | Bounding visual container for listbox of options |   | .slds-dueling-list div |   |   |
| .slds-dueling-list__column_responsive | Changes the layout of the dueling picklist to be responsive |   | .slds-dueling-list__column |   |   |
| .slds-dueling-list__column | Handles the layout of the dueling picklist |   | .slds-dueling-list div |   |   |
| .slds-dueling-list | Initializes a dueling picklist | dev-ready | div | true |   |
| .slds-dueling-list | A dueling-picklist is used to move options between two lists and is often referred to as a multi-select. Sometimes, the list options can then be re-ordered, depending on the use case. | dev-ready |   |   |   |